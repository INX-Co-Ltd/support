...
- All communication on this path happens **directly between nearby devices** and is **not routed through any external server**. If a direct connection cannot be established and your transport mode includes iCloud, the app delivers the share through Apple's CloudKit instead (see "iCloud Data"); in Nearby-only mode it keeps retrying the direct connection and does not fall back to iCloud.
